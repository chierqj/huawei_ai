...
class DoBeat(Action):
    ...
    def do_excute(self):
        ...
        for k, player in mPlayers.iteritems():
            ...
            for m, x, y in next_one_points:
                extend_points = self.get_extend_points(x, y)
                weight = 0
                for step, ex, ey in extend_points:
                    if True == self.match_have_othplayer(ex, ey):
                        weight += 1.0 / math.exp(step)
                if max_weight == None or weight < max_weight:
                    max_weight, ret_move = weight, m
                self.weight_moves[m] = weight
            player.move = ret_move
            self.record_detial(player)
